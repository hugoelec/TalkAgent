...
QasrGbxOmni 是一個本機語音對話與讀書機實驗工具，把麥克風收音、ASR、LLM 回覆、OmniVoice TTS、Web UI、對話記憶與 EPUB 讀書模式串在同一個 Python/Flask 應用裡。
...
- 讀書機 / BookReader / Reader Mode 支援上傳 EPUB，抽取 XHTML 章節、分析書籍內容、保存 `AnalyzeResault` 與 `SummaryIndex` 為可下載文字包，並讓語音問答只根據書籍分析內容回答。
...
- `BookReader`: 讀書機工作區，可上傳 EPUB、檢視 XHTML 章節、執行分析、下載 BookReader 文字包。
- `OmniVoice Settings`: 可即時調整部分 TTS 參數，修改後會自動送到 runtime 設定。

## 讀書機 / BookReader / Reader Mode

讀書機目前使用 EPUB 作為入口，目標是把一本書先拆成可分析的章節，再把分析結果變成後續語音問答的上下文：
...
讀書機相關 prompt 放在 `ControlPrompt.yaml` 的 `ReaderConfig`，包含載入書籍、開始閱讀、閱讀中、結束閱讀與章節分析用的提示詞。一般聊天模式與讀書機模式會走不同 prompt 組裝路徑，避免讀書問答混到平常聊天記憶。
